        ...
        for(ProductionRule pr: prs.second) {
            ...
            for(int i = 0; i < (int)tokens.size() - 1 ; ++i) {
                if(tokens[i].getValue() == state) {
                    ...
                    if(tokens[i+1].getType() == RuleTokenType::NON_TERMINAL) 
                    	cur_first = getFirst(tokens[i+1].getValue());
            ...
            for(int i = (int) tokens.size()-1 ; i >= 0 ; --i) {
                ...
                if(tokens[i].getType() == RuleTokenType::NON_TERMINAL && 
                	!getFirst(tokens[i].getValue()).count(Constants::LAMBDA))
                    break;
